...
def process_youtube():
    ...
    try:
        ...
        if not original_entries:
            return jsonify({"error": "影片沒有字幕"}), 400

        src_lang_code = transcript.language_code  # 例如 'en', 'ja'
        # 将语言代码映射为我们的格式（如 'en' -> 'en', 'ja' -> 'ja'）
        # 直接使用原代码，翻译函数能够处理
        translated_entries = []
        for entry in original_entries:
            original_text = entry['text'].strip()
            if original_text:
                translated_text = translate_subtitle_text(
                    original_text, src_lang_code, target_lang,
                    gemini_key, deepl_key
                )
            else:
                translated_text = ""
